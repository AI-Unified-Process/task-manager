...
import ch.martinelli.demo.aiup.core.ui.components.Notifier;
...
public class UserView extends Div implements HasUrlParameter<String>, HasDynamicTitle {
	...
	private final Button cancel = new Button(getTranslation("common.cancel"));

	private final Button save = new Button(getTranslation("common.save"));
	...
	public String getPageTitle() {
		return getTranslation("user.title");
	}
	...
	private VerticalLayout createGrid() {
		...
		grid.addColumn(u -> u.getUser().getLastName())
			...
			.setAutoWidth(true);
		grid.addColumn(u -> String.join(", ", u.getRoles())).setHeader(getTranslation("user.roles")).setAutoWidth(true);
		...
		grid.addComponentColumn(u -> {
			...
			deleteIcon.addClickListener(_ -> new ConfirmDialog(getTranslation("user.deleteConfirmTitle"),
					getTranslation("user.deleteConfirmMessage", u.getUser().getUsername()),
					getTranslation("common.delete"), confirmEvent -> {
						userDAO.deleteUserAndRolesByUsername(u.getUser().getUsername());
						clearForm();
						refreshGrid();
					}, getTranslation("common.cancel"), cancelEvent -> {
					})
				.open());
			return deleteIcon;
	...
	private VerticalLayout createForm() {
		...
		var firstNameField = new TextField(getTranslation("user.firstName"));
		binder.forField(firstNameField)
			.asRequired()
			.bind(u -> u.getUser().getFirstName(), (u, s) -> u.getUser().setFirstName(s));

		var lastNameField = new TextField(getTranslation("user.lastName"));
		binder.forField(lastNameField)
			.asRequired()
			.bind(u -> u.getUser().getLastName(), (u, s) -> u.getUser().setLastName(s));

		var passwordField = new PasswordField(getTranslation("login.password"));
		...
		var roleMultiSelect = new MultiSelectComboBox<String>(getTranslation("user.roles"));
		...
		save.addClickListener(_ -> {
			...
			if (user != null && validationStatus.isOk()) {
				try {
					...
					try {
						userDAO.save(user);
						Notifier.success(getTranslation("user.savedNotification"));
					}
					catch (DataAccessException _) {
						Notifier.error(getTranslation("user.saveErrorNotification"));
					}
				}
				catch (ValidationException ex) {
					Notifier.error(getTranslation("common.validationErrors"));
